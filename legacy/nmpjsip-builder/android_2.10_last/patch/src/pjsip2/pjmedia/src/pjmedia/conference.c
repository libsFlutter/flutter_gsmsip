	...
	while (cport->rx_buf_count < samples_req) {
		...
		*type = PJMEDIA_FRAME_TYPE_AUDIO;
		...
		if (cport->channel_count == 1) {
		    //A// pjmedia_convert_channel_1ton((pj_int16_t*)f.buf, 
			pjmedia_convert_channel_r1ton((pj_int16_t*)f.buf, 
			...
			//A// pjmedia_convert_channel_nto1((pj_int16_t*)f.buf, 
		    pjmedia_convert_channel_rnto1((pj_int16_t*)f.buf, 
						 (const pj_int16_t*)f.buf,
						 cport->channel_count, 
						 cport->samples_per_frame, 
			      ...
			      pjmedia_frame_type *frm_type)
{
	...
	*frm_type = PJMEDIA_FRAME_TYPE_NONE;
	...
	if (conf->channel_count == 1) {
	    //A// pjmedia_convert_channel_1ton(tx_buf, tx_buf,
		pjmedia_convert_channel_r1ton(tx_buf, tx_buf,
					 cport->channel_count, 
					 dst_count, 0);
	    dst_count *= cport->channel_count;
	} else { /* cport->channel_count == 1 */
	    //A// pjmedia_convert_channel_nto1(tx_buf, tx_buf,
		pjmedia_convert_channel_rnto1(tx_buf, tx_buf,
					 conf->channel_count, 
