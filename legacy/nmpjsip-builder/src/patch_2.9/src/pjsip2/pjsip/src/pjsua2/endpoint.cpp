...
void MediaConfig::fromPj(const pjsua_media_config &mc)
{
    ...
    //A//
    //this->headphonesOnly = PJ2BOOL(mc.headphones_only);
    this->headphonesOnly = mc.headphones_only;
    this->micSource = mc.mic_source;
    this->streamType = mc.stream_type;
    this->adjustTx = mc.adjust_tx;
    this->adjustRx = mc.adjust_rx;
...
pjsua_media_config MediaConfig::toPj() const
{
    ...
    //A//
    mcfg.headphones_only = this->headphonesOnly;
    mcfg.mic_source = this->micSource;
    mcfg.stream_type = this->streamType;
    mcfg.adjust_tx = this->adjustTx;
    mcfg.adjust_rx = this->adjustRx;
...
void MediaConfig::readObject(const ContainerNode &node) PJSUA2_THROW(Error)
{
    ...
    //A//
    //NODE_READ_BOOL    ( this_node, headphonesOnly);
    NODE_READ_UNSIGNED( this_node, headphonesOnly);
    NODE_READ_UNSIGNED( this_node, micSource);
    NODE_READ_UNSIGNED( this_node, streamType);
    NODE_READ_UNSIGNED( this_node, adjustTx);
    NODE_READ_UNSIGNED( this_node, adjustRx);
...
void MediaConfig::writeObject(ContainerNode &node) const PJSUA2_THROW(Error)
{
    ...
    //A//
    //NODE_WRITE_BOOL    ( this_node, headphonesOnly);
    NODE_WRITE_UNSIGNED( this_node, headphonesOnly);
    NODE_WRITE_UNSIGNED( this_node, micSource);
    NODE_WRITE_UNSIGNED( this_node, streamType);
    NODE_WRITE_UNSIGNED( this_node, adjustTx);
    NODE_WRITE_UNSIGNED( this_node, adjustRx);
