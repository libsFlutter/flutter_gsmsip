 ...
 * @return		    PJ_SUCCESS on success;
 */
PJ_INLINE(pj_status_t) pjmedia_convert_channel_rnto1(pj_int16_t mono[],
						    const pj_int16_t multi[],
						    unsigned channel_count,
						    unsigned samples_per_frame,
						    pj_bool_t mix,
 ...
 * @return		    PJ_SUCCESS on success;
 */
PJ_INLINE(pj_status_t) pjmedia_convert_channel_r1ton(pj_int16_t multi[],
						    const pj_int16_t mono[],
						    unsigned channel_count,
						    unsigned samples_per_frame,
						    unsigned options)
{
	...
	for (i=1; i<=channel_count; ++i)
	{
		//A//multi[samples_per_frame-i] = *src;
	    //multi[samples_per_frame-i] = (i==1?(*src):(*src^0xffff));
	    //multi[samples_per_frame-i] = (i==1?(*src):0);
	    //multi[samples_per_frame-i] = (i!=1?(-(*src)):0);
	    //multi[samples_per_frame-i] = 0;
	    //(pj_int16_t) tmp -
	    multi[samples_per_frame-i] = (i==1?(*src):-(*src));
	}
	samples_per_frame -= channel_count;
	--src;
    }

    //PJ_LOG(4, ("Sip stereo", "r2 channel_count: %d", channel_count));
    return PJ_SUCCESS;
}
 ...
 * @return		    PJ_SUCCESS on success;
 */
PJ_INLINE(pj_status_t) pjmedia_convert_channel_1tolr(pj_int16_t multi[],
						    const pj_int16_t mono[],
						    unsigned channel_count,
						    unsigned samples_per_frame,
						    unsigned options, unsigned left)
{
    const pj_int16_t *src;
	...
	for (i=1; i<=channel_count; ++i)
	{
		...
		if (left)
	    	...
	    	multi[samples_per_frame-i] = (i==1?0:(*src));
 ...
 */
/*
PJ_INLINE(pj_status_t) pjmedia_convert_channel_1tof(float multi[],
						    const pj_int16_t mono[],
						    unsigned channel_count,
						    unsigned samples_per_frame,
						    unsigned options, unsigned left)
