...
//A//
            ...
            //PJ_LOG (4, (THIS_FILE, "size:%d",
            //                       size));
/*
        frame.size =  size*2;

        frame.buf=frame_buf;
        pjmedia_convert_channel_r1ton(frame.buf, (void *)buf,
					 2, 
					 size, 0);
*/
...
static int AndroidTrackCallback(void *userData)
{
    ...
    //A// outputBuffer = (*jni_env)->NewByteArray(jni_env, size);
    ...
    while (!stream->quit_flag) {
        ...
        //A//
        frame.buf=frame_buf;
        pjmedia_convert_channel_r1ton(frame_buf,frame.buf,
					 2, 
					 size, 0);
        //A//^^
        ...
        //A// (*jni_env)->ReleaseByteArrayElements(jni_env, outputBuffer, buf,
        (*jni_env)->ReleaseByteArrayElements(jni_env, outputBuffer, frame_buf,
        				     JNI_COMMIT);
        ...
        bytesWritten = (*jni_env)->CallIntMethod(jni_env, stream->track,
                                                 ...
                                                 //A// 0, size);
                                                 0, size*2);
        //A// if (bytesWritten <= 0 || bytesWritten != size) {
        if (bytesWritten <= 0 || bytesWritten != size*2) {
                                         ...
                                         pjmedia_aud_stream **p_aud_strm)
{
    ...
    //A//
    //channelInCfg = 16 /*CHANNEL_IN_MONO*/;

    channelOutCfg =  12 /*CHANNEL_OUT_STEREO*/;
	    ...
	    (param->input_route & PJMEDIA_AUD_DEV_ROUTE_CUSTOM))
	{
	    ...
	    char sdk_version[PROP_VALUE_MAX];
            ...
            */
            ...
            //mic_source = 7; /* VOICE_COMMUNICATION */ // Android 7 Working
    ...
    if (stream->dir & PJMEDIA_DIR_PLAYBACK) {
        ...
        track_obj = (*jni_env)->NewObject(jni_env,
                                          ...
                                          1, /* 1 - STREAM_SYSTEM */
                                          //0, /* 0 - STREAM_VOICE_CALL */ //A// REDMI WORKING
                                          param->clock_rate,
                                          channelOutCfg,
                                          sampleFormat,
                                          inputBuffSizePlay,
