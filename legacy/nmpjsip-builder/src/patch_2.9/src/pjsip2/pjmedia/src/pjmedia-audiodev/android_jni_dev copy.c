...
static int AndroidTrackCallback(void *userData)
{
    ...
    while (!stream->quit_flag) {
        ...
        if(stream->param.headphones_only)
        {
            ...
            pjmedia_convert_channel_r1ton((void *)buf,(void *)buf2,
                        2, 
                       size*2, 0); //Странно, не совсем ясно почему. Возможно размер буффера в байтах, а тут в инт
            //            size, 0);
            //            size/2, 0);
            //????
        ...
        {
            PJ_LOG(4, (THIS_FILE, "headphonesOnly ON"));
            PJ_LOG(4, (THIS_FILE, "size*2=%d",size*2));
