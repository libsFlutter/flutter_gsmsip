...
import '../sources/voice_line/tty_port_source.dart';
import '../sources/voice_line/enhanced_mode_source.dart';
import '../sources/voice_line/dongle_source.dart';
import '../sources/voice_line/telecom_api_source.dart';
...
class VoiceLineRepositoryImpl implements VoiceLineRepository {
  ...
  Future<List<VoiceLineMethodStatus>> getAvailableMethods() async {
    try {
      ...
      final acousticStatus = await _getAcousticStatus();
  ...
  Future<VoiceLineMethodStatus> getMethodStatus(VoiceLineMethod method) async {
    switch (method) {
      ...
      case VoiceLineMethod.acoustic:
        return await _getAcousticStatus();
    }
  }
  ...
  Future<TestMethodResult> _testTtyMethod() async {
    ...
    final ttyResult = await ttyPortSource.testPort(portPath, baudRate);
    return TestMethodResult(
      success: ttyResult.success,
      error: ttyResult.error,
      measurements: {'latencyMs': ttyResult.latencyMs, 'response': ttyResult.response},
      quality: ttyResult.success ? QualityLevel.great : QualityLevel.poor,
    );
  }
