...
class SmsService {
  ...
  /// Send SMS - always via local GSM telephony
  /// SMPP is receive-only for incoming SMS commands
  Future<bool> sendSms(String recipient, String content) async {
    final result = await sendSmsLocal(recipient, content);
    return result != null;
  }

  /// Send SMS via SMPP
  Future<String?> sendSmsViaSmpp(String recipient, String content) async {
    if (_smppConnectionState != SmppConnectionState.connected) {
